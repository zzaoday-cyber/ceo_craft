...
st.set_page_config(page_title="CEO Craft - 매출", layout="wide")
...
st.markdown("매출 시뮬레이션 입력값을 설정하세요.")

with st.form("sales_simulation_form"):
	st.subheader("상품 설정")
	...
	submitted = st.form_submit_button("매출 시뮬레이션")
...
if submitted:
	...
	try:
		...
		col1.metric("주문건", f"{result.orders:,.2f}")
		col1.metric("매출", f"{result.revenue:,.0f}")
		col2.metric("비용", f"{result.expenses:,.0f}")
		col2.metric("현금흐름(다음)", f"{result.cash_flow_next:,.0f}")
		col3.metric("매출실적(다음)", f"{result.total_revenue_next:,.0f}")
		col3.metric("재고량(다음)", f"{result.inventory_next:,.2f}")

		st.markdown("---")
		st.write("이탈율", f"{result.churn_rate:.4f}")
		st.write("전환율", f"{result.conversion_rate:.4f}")
