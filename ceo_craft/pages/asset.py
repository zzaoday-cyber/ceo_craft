...
st.set_page_config(page_title="CEO Craft - 자산", layout="wide")
...
st.markdown("자산 시뮬레이션 입력값을 설정하세요.")
...
with st.form("asset_simulation_form"):
	...
	equity = st.number_input("자본", min_value=0.0, value=0.0, step=100000.0)
	debt = st.number_input("부채", min_value=0.0, value=0.0, step=100000.0)

	submitted = st.form_submit_button("자산 시뮬레이션")
...
if submitted:
	...
	try:
		...
		col1.metric("자산", f"{assets:,.0f}")
		col1.metric("자본", f"{equity:,.0f}")
		col2.metric("부채", f"{debt:,.0f}")
		col2.metric("현금흐름(다음)", f"{result.cash_flow_next:,.0f}")

		st.markdown("---")
		st.write("매출", f"{result.revenue:,.0f}")
		st.write("비용", f"{result.expenses:,.0f}")
		st.write("재고량(다음)", f"{result.inventory_next:,.2f}")
