...
st.set_page_config(page_title="CEO Craft - 손익", layout="wide")
...
st.markdown("손익 시뮬레이션 입력값을 설정하세요.")

with st.form("profit_simulation_form"):
	st.subheader("상품 설정")
	...
	submitted = st.form_submit_button("손익 시뮬레이션")
...
if submitted:
	...
	try:
		...
		st.subheader("손익 결과")
		profit_value = result.revenue - result.expenses

		col1, col2 = st.columns(2)
		col1.metric("매출", f"{result.revenue:,.0f}")
		col1.metric("비용", f"{result.expenses:,.0f}")
		col2.metric("손익", f"{profit_value:,.0f}")
		col2.metric("현금흐름(다음)", f"{result.cash_flow_next:,.0f}")

		st.markdown("---")
		st.write("주문건", f"{result.orders:,.2f}")
