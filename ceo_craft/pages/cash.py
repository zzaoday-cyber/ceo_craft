...
st.set_page_config(page_title="CEO Craft - 현금", layout="wide")
...
st.markdown("현금흐름 시뮬레이션 입력값을 설정하세요.")

with st.form("cash_simulation_form"):
	st.subheader("상품 설정")
	...
	submitted = st.form_submit_button("현금흐름 시뮬레이션")
...
if submitted:
	...
	try:
		...
		st.subheader("현금흐름 결과")
		col1, col2 = st.columns(2)
		col1.metric("현금흐름(현재)", f"{cash_flow:,.0f}")
		col1.metric("현금흐름(다음)", f"{result.cash_flow_next:,.0f}")
		col2.metric("비용", f"{result.expenses:,.0f}")
		col2.metric("매출", f"{result.revenue:,.0f}")
